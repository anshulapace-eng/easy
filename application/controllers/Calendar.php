...
class Calendar extends EA_Controller
{
    ...
    private function get_user_setting(int $user_id, string $setting_name, $default = null)
    {
        ...
        if ($role_slug === DB_SLUG_SECRETARY) {
            ...
            if (!empty($secretary['providers'])) {
                $user_id = (int) $secretary['providers'];
            }
        }
        ...
        $setting = $this->db->where('id_user', $user_id)
        ...
        return setting($setting_name) ?: $default;
    }
    ...
    public function index(string $appointment_hash = ''): void
    {
        ...
        // Fetch user-specific business/appointment settings based on logged-in user ID
        $appointment_status_options = $this->get_user_setting($user_id, 'appointment_status_options');
        $company_working_plan = $this->get_user_setting($user_id, 'company_working_plan');
