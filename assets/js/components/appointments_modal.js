...
App.Components.AppointmentsModal = (function () {
    ...
    const $timeSlotDate = $('#start-datetime');
    const $timeSlotSelect = $('#end-datetime');
    ...
    function buildTimeSlotOption(slot) {
        const option = $('<option/>', {
            value: slot.start,
            text: slot.label,
        ...
        if (!slot.available) {
            option.addClass('text-muted bg-light');
        }

        return option;
    }

    function populateTimeSlots(slots, selectedStartTime = null) {
        ...
        if (slots.length === 0) {
            $timeSlotSelect.append(
                $('<option/>', {
                    value: '',
                    text: 'No slots available this date',
                    disabled: true,
                }),
            );
            return;
        ...
        slots.forEach((slot) => {
            const option = buildTimeSlotOption(slot);

            if (selectedStartTime && slot.start === selectedStartTime) {
    ...
    function loadTimeSlotsForDate(date, selectedStartTime = null) {
        if (!date) {
            populateTimeSlots([]);
            return;
        }

        $timeSlotSelect.html('<option value="" disabled selected>Loading slots...</option>');

        $.ajax({
            url: App.Utils.Url.siteUrl('calendar'),
            type: 'GET',
            data: { date },
            dataType: 'json',
        })
            .done((slots) => {
                populateTimeSlots(slots, selectedStartTime);
            })
            .fail(() => {
                $timeSlotSelect.empty();
                $timeSlotSelect.append(
    ...
    function addEventListeners() {
        ...
        $saveAppointment.on('click', () => {
            ...
            const appointmentDate = $timeSlotDate.val();
            const selectedSlotOption = $timeSlotSelect.find('option:selected');
            const appointmentStartTime = selectedSlotOption.val();
            const appointmentEndTime = selectedSlotOption.data('slot-end');

            const startDatetime = `${appointmentDate} ${appointmentStartTime}:00`;
            const endDatetime = `${appointmentDate} ${appointmentEndTime}:00`;
    ...
    function resetModal() {
        ...
        $timeSlotDate
            .data('selected-start-time', null)
            .val(today);

        loadTimeSlotsForDate(today);
    ...
    function validateAppointmentForm() {
        ...
        try {
            ...
            const appointmentDate = $timeSlotDate.val();
            const appointmentStartTime = $timeSlotSelect.val();
            const appointmentEndTime = $timeSlotSelect.find('option:selected').data('slot-end');

            if (!appointmentDate || !appointmentStartTime || !appointmentEndTime) {
                $timeSlotDate.addClass('is-invalid');
                $timeSlotSelect.addClass('is-invalid');
                throw new Error(lang('start_date_before_end_error'));
            }

            const startDatetime = new Date(`${appointmentDate}T${appointmentStartTime}:00`);
